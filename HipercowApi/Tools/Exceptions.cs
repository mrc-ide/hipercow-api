...
namespace Hipercow_api.Tools.Exceptions
{
    ...
    public class LdapAuthFailure(string user) :
        Exception($"Couldn't authenticate user {user}")
    {
    }
    ...
    public class LdapNoClusterPermissions(string user) :
        Exception($"User {user} does not have permission to use any clusters.")
    {
    }
    ...
    /// </summary>
    public class LdapEmptyUsernamePassword() :
        Exception("User and password for login cannot be empty.")
    {
    }
}